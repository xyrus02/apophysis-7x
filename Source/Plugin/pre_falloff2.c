...
typedef struct {
    double pre_falloff2_scatter;
    double pre_falloff2_mindist;
	
	double pre_falloff2_mul_x;
	double pre_falloff2_mul_y;
	double pre_falloff2_mul_z;
	double pre_falloff2_mul_c;
	
    double pre_falloff2_x0;
    double pre_falloff2_y0;
	double pre_falloff2_z0;
	
	int pre_falloff2_invert;
	int pre_falloff2_type;
...
APO_PLUGIN("pre_falloff2");
APO_VARIABLES(
    VAR_REAL_RANGE(pre_falloff2_scatter, EPS, DBL_MAX, 1.0),
    VAR_REAL_RANGE(pre_falloff2_mindist, 0.0, DBL_MAX, 0.5),
	
	VAR_REAL_RANGE(pre_falloff2_mul_x, 0.0, 1.0, 1.0),
	VAR_REAL_RANGE(pre_falloff2_mul_y, 0.0, 1.0, 1.0),
	VAR_REAL_RANGE(pre_falloff2_mul_z, 0.0, 1.0, 0.0),
	VAR_REAL_RANGE(pre_falloff2_mul_c, 0.0, 1.0, 0.0),
	
    VAR_REAL(pre_falloff2_x0, 0.0),
    VAR_REAL(pre_falloff2_y0, 0.0),
	VAR_REAL(pre_falloff2_z0, 0.0),
	
	VAR_INTEGER_RANGE(pre_falloff2_invert, 0, 1, 0),
	VAR_INTEGER_RANGE(pre_falloff2_type, 0, 2, 0),
);

int PluginVarPrepare(Variation* vp)
{
	double4 m = { 
		param(pre_falloff2_mul_x), 
		param(pre_falloff2_mul_y), 
		param(pre_falloff2_mul_z),
		param(pre_falloff2_mul_c) }; 
	double3 c = { 
		param(pre_falloff2_x0), 
		param(pre_falloff2_y0), 
		param(pre_falloff2_z0) }; 

	param(r_max) = scatter_adjust * param(pre_falloff2_scatter);
	param(d) = param(pre_falloff2_mindist);
	
	param(mul) = m;
	param(center) = c;
	
	param(invert) = param(pre_falloff2_invert);
	param(type) = param(pre_falloff2_type);
